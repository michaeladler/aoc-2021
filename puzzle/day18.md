...
You descend into the ocean trench and encounter some [snailfish][1]. They say they saw the sleigh keys! They'll even tell you which direction the keys went if you help one of the smaller snailfish with his *math homework*.
...
During reduction, at most one action applies, after which the process returns to the top of the list of actions. For example, if *split* produces a pair that meets the *explode* criteria, that pair *explodes* before other *splits*
occur.
...
The homework assignment involves adding up a *list of snailfish numbers* (your puzzle input). The snailfish numbers are each listed on a separate line. Add the first snailfish number and the second, then add that result and the third,
...
Note that snailfish addition is not [commutative][2] - that is, `x + y` and `y + x` can produce different results.
...
[1] https://en.wikipedia.org/wiki/Snailfish
[2] https://en.wikipedia.org/wiki/Commutative_property
[3] /2021
[4] 18/input
[5] https://twitter.com/intent/tweet?text=I%27ve+completed+%22Snailfish%22+%2D+Day+18+%2D+Advent+of+Code+2021&url=https%3A%2F%2Fadventofcode%2Ecom%2F2021%2Fday%2F18&related=ericwastl&hashtags=AdventOfCode
[6] javascript:void(0);
